...
const x = (base_1, base_2 = base_1) => base_1 * base_2 % P;
const o = (condition, one, two) => -condition & (one ^ two);
const t = (base, power) => {
    do
        base = x(base);
    while (--power);
...
const i = (bytes) => {
    let a = 0n, z = 0, y = 0n;
    do
        a |= BigInt(bytes[z] ?? 0) << y;
    while (y += 8n, ++z < 32);
    return a;
};
export const x25519 = (key_1, key_2) => {
    let a = new Uint8Array(32), b, c = 0, z = 0;
    if (key_2)
        a.set(key_2.subarray(0, 32)), b = i(a);
